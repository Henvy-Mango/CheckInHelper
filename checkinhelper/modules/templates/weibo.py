...
class SuperTopicCheckin(object):
    ...
    def get_follow_data(self):
        ...
        if errno:
            raise CheckinHelperException(f'Failed to get follow super:\n{response}')
        ...
        if not follow_list:
            raise CheckinHelperException('Failed to get follow list, '
        ...
        for follow in follow_data:
            ...
            if is_sign == '已签':
                ...
                # Already checked in
                    ...
                    raise CheckinHelperException(f'{name_style} :Failed to run sign:\n{response}')
            else:
                raise CheckinHelperException('Unknown check-in status')
...
class RedemptionCode(object):
    ...
    def mybox_codes(self):
        ...
        if response.status_code != 200:
            raise CheckinHelperException('Failed to get my box codes: '
    ...
    def check_event(self, subscribe=False):
        ...
        if subscribe:
            ...
            send2all(status=message[0], desp=message[1])
    ...
    def get_code(self, id):
        ...
        if response.get('k'):
            ...
            raise CheckinHelperException('Failed to get code: '
                                         'The cookie seems to be invalid, please re-login to m.weibo.cn')
        else:
