...
from checkinhelper.modules.templates import journal
from checkinhelper.tools.config import config
from checkinhelper.tools.exceptions import CookiesExpired
from checkinhelper.tools.utils import log, request, get_ds
...
def _data_handler(func):
    ...
    def wrapper(self, *args, **kwargs):
        if 'Yuanshen' in self.__class__.__name__:
            ...
            if role_list:
                result_list = []
                log.info('当前账号绑定了 {num_roles} 个角色'.format(
...
class __BaseCheckin(object):
    ...
    🌕摩拉: {month_mora}''').format(**ledger)
        ...
        log.info('准备为旅行者 {hidden_uid} 签到...'.format(
