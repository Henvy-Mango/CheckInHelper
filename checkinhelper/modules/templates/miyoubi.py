...
from checkinhelper.tools.utils import log, request, get_ds, extract_cookie
...
def get_app_cookie(cookie):
    ...
    if response.get('message') != 'OK':
        raise CheckinHelperException('Failed to get multi token: '
    ...
    raise CheckinHelperException(f'YOUR COOKIE_MIYOUBI:\n{app_cookie}')
...
class MiyoubiCheckin(object):
    ...
    def get_posts(self, forum_id: int):
        ...
        message = response.get('message')
        if message != 'OK':
            raise CheckinHelperException(f'Failed to get posts:\n{message}')
        ...
        log.info('Succeeded in getting {num_posts} posts'.format(
            num_posts=len(posts)
        ))
        return posts
